...
prepare_local_partition() {
  ...
  MOUNT_POINT=$(mount -v | grep "/dev/sda3" | awk '{print $3}' ||:)

  if [[ x"${MOUNT_POINT}" == x"${MOUNT_DIR}" ]];then
    record_log "/dev/sda3 is already mounted on ${MOUNT_POINT}"
    return
  fi

  if [ x$(sudo file -sL /dev/sda3 | grep -o ext4) == x"" ]; then
    create_extfs;
  fi
...
clone_cheri_repos() {
  ...
  sudo apt install -y autoconf automake libtool pkg-config clang bison cmake mercurial ninja-build samba flex texinfo time libglib2.0-dev libpixman-1-dev libarchive-dev libarchive-tools libbz2-dev libattr1-dev libcap-ng-dev libexpat1-dev libgmp-dev libssl-dev
}
